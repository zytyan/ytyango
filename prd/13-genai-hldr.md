...
| FR-4 | 在 JS 环境中提供 `reply(obj)` 函数，输出内容会作为新的对话消息附加到 session，并在下一轮 prompt 中带上。 | 高 |
| FR-5 | 为 JS 执行设置资源限制（单次执行超时、最大输出大小、禁用标准库危险接口），执行错误需捕获并回传用户，同时写入日志。 | 高 |
...
* 新建 `genai_hldr` 包（位置：`myhandlers/genai_hldr` 或同级子目录），内部包含：
  ...
  * 提示词与 Tool 配置生成：系统提示词通过模板渲染（预置占位符：当前时间、chat 名称/类型、bot 名称与 username、工具说明），可读写独立模板文件或嵌入多语言模板库，便于维护与扩展。
  * 模型调用适配器，包装 `genai.Client` 与重试/超时。
  * JS 执行子模块，封装 goja Runtime 复用、`reply()` 注入、脚本截取、输出累积。
...
* 模型输出的 `//execjs` 段可被正确解析并执行，`reply()` 结果会体现在后续对话上下文；错误/超时有明确提示。
