...
| FR-11 | 提供聊天消息搜索能力：支持在存量会话消息中按关键词/用户名查询，模型可通过调用工具获取最近匹配结果，并在回复中引用。 | 中 |
...
* 新建 `genai_hldr` 包（位置：`myhandlers/genai_hldr` 或同级子目录），内部包含：
  ...
  * 消息搜索工具：封装聊天记录查询（按关键词/用户名，限制返回条数），供模型通过工具调用获取最近匹配消息摘要。
...
* 消息搜索工具可按关键词/用户名返回受限条数的最新匹配消息摘要，并可被模型用于生成回复。
