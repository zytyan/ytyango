...
* 对 Telegram 不支持的 Markdown 元素，按既定降级策略保持信息完整：
  * 图片转为链接文本；有序列表保留数字，无序列表用圆点直接输出；表格整体收拢为无语言标记的代码块；公式转为行内代码；发送时依赖实体，无需额外转义特殊字符。
...
| FR-8 | 输出 text 依赖实体标注，不额外转义特殊字符（Telegram 会按实体解释）。                          | 高 |
...
| FR-10 | 有序列表保留数字，无序列表统一用圆点符号，直接文本输出（不包代码块）。                           | 高 |
| FR-11 | 公式（行内/块）：转为行内代码实体，内容原样保留。                                      | 高 |
| FR-12 | 表格、任务列表等其他不支持结构：以原始行文本加入代码块或行内代码，保持可读性。                      | 中 |
...
* Entity 生成：统一中间表示，集中处理 UTF-16 偏移与降级（无需额外字符转义）。
* 降级策略：在 AST 层识别 unsupported nodes，落到代码块/行内代码；列表聚合文本输出；图片转链接。
* 测试：样例覆盖多 emoji、混合语言、实体偏移、代码块原样保留、列表降级、公式降级。
...
| Week 3 | 完成降级策略（列表聚合文本输出、图片转链接、公式降级、表格处理），补充多语言/emoji 偏移单测。 |
