...
| ID | 描述 | 优先级 |
| --- | --- | --- |
| FR-5 | 为现有所有表提供 PostgreSQL DDL：将 `INT_BOOL` → `boolean`，`INT_UNIX_SEC` → `bigint`（保留秒计数），`JSON_TEXT` → `jsonb`，`BLOB/BLOB_*` → `bytea`，自增字段用 `bigserial`/`generated by default as identity`。 | 高 |
...
CREATE TABLE users (
    ...
    last_name        text,
    profile_update_at bigint    NOT NULL,
    profile_photo    text,
