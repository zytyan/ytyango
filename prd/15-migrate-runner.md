...
7. **作为维护者，我可以通过 `--memory-run` 将当前数据库快速复制到内存，抽样部分数据后演练迁移，避免影响真实磁盘库。**
...
| FR-10 | 提供 `--memory-run` 选项，在内存数据库镜像上模拟迁移（可抽样部分数据），不影响磁盘库 | 中 |
| FR-11 | `--memory-run` 支持采样比例或行数配置，默认抽样子集以降低内存占用 | 低 |
...
* 性能：迁移过程中避免长时间全表锁，必要时提供提示或分步策略；`--memory-run` 支持抽样以降低内存占用与耗时。
...
* 内存演练：`--memory-run` 从目标数据库复制到 `:memory:`（支持按表/行抽样），在内存库上实际执行迁移以验证可行性与耗时，最终不落盘。
...
* 内存演练：`./ytyango migrate up --target main --memory-run --sample-rate 0.2`（将当前库复制到内存，仅抽样 20% 数据做演练）
