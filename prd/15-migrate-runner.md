...
7. **作为维护者，我可以通过 `--memory-run` 将当前数据库的所有表快速复制到内存，按行抽样部分数据后演练迁移，避免影响真实磁盘库。**
...
| FR-10 | 提供 `--memory-run` 选项，在内存数据库镜像上模拟迁移（复制所有表结构与索引，支持行级抽样数据），不影响磁盘库 | 中 |
| FR-11 | `--memory-run` 支持采样比例或行数配置（对所有表应用统一采样策略），默认抽样子集以降低内存占用 | 低 |
| FR-12 | 命令行输出详细日志（执行 SQL、耗时、dry-run 展示），并返回非 0 退出码于失败 | 中 |
| FR-13 | 仅内置 SQLite 驱动，避免引入其他数据库依赖以控制二进制体积 | 低 |
| FR-14 | 裸 `ytyango migrate` 或缺少子命令时输出帮助/子命令列表，提示默认配置来源与示例用法 | 低 |
...
* 可靠性：同一版本迁移失败时标记 dirty，下一次执行可检测到并先回滚/清理后重试；尽量使用事务保持原子性。单机场景，无需额外锁管理。
* 可维护性：迁移版本号、描述与变更点集中声明，提供模板/规范，避免重复代码。
* 可测试性：提供单元测试覆盖版本检测、dry-run 输出、成功/失败场景。
* 性能：迁移过程中避免长时间全表锁，必要时提供提示或分步策略；`--memory-run` 全表复制结构、行级抽样数据以降低内存占用与耗时。
...
* 原子性策略：默认单事务执行一个版本的所有 Up/Down SQL；遇到 SQLite 不支持的多语句事务变更（如某些 `ALTER TABLE`）时，使用可回滚的临时表方案，并在 dirty 记录中注明。单机场景，无需额外分布式锁。
* DRY RUN：执行器在 dry-run 时不写库，打印执行顺序、SQL、预估影响表，并标记将写入的版本变化。
* 内存演练：`--memory-run` 从目标数据库复制所有表结构与索引到 `:memory:`，对行数据按统一采样规则抽样后在内存库上实际执行迁移，以验证可行性与耗时，最终不落盘。
* CLI：在 `cmd` 或主入口中新增 `migrate` 子命令，支持 `--target main|msg|all`、`--db`（路径，覆盖配置）、`--to`、`--step`、`--dry-run`、`--force-down`、`--memory-run`、采样参数（如 `--sample-rate` 或 `--sample-rows`）等 flag；裸命令输出帮助。
...
* 内存演练：`./ytyango migrate up --target main --memory-run --sample-rate 0.2`（将当前库所有表复制到内存，行级抽样 20% 数据做演练）
