...
- [ ] 后端入口与路由迁移
  ...
  - [ ] 入口需要暴露/注入 bot 供头像下载，并与主体服务的启动流程对齐（替换旧 `bothttp` 路由）。
...
- [ ] 搜索接口实现
  ...
  - [ ] 修正 `ins_id` 未命中/查询错误时返回 400（目前会继续调用 Meili）。
  - [ ] clamp `limit` 到 1..50，与 PRD 一致。
...
- [ ] 用户信息批量接口实现
  ...
  - [ ] 返回结构需填充 `name`（必填）与 `username`（可空），错误 ID 应产生整体 400 而非 200 内嵌 error。
...
- [ ] 用户头像接口实现
  ...
  - [ ] 校验 tgauth 与路径 `userId` 绑定，且验签密钥与 PRD（`sha256(botToken)`）一致。
