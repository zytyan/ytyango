...
func saveMessage(bot *gotgbot.Bot, ctx *ext.Context) (err error) {
	...
	mongoId, err := uuid.NewUUID()
	if err != nil {
		log.Errorf("Generate uuid error %s, update id %d", err, ctx.Update.UpdateId)
		return errors.New("insert mongodb error")
	}
	meiliMsg := &MeiliMsg{
		MongoId:   strings.Replace(mongoId.String(), "-", "", -1),
	...
	post, err := saveMsgMeili.Post(meili.GetSaveUrl(), "application/json", bytes.NewReader(j))
	if err != nil {
