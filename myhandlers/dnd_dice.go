...
var setAttrRe = regexp.MustCompile(`^([a-zA-Z0-9\p{Han}\p{Hiragana}\p{Katakana}]{1,20})\s*(\+=?|-=?|=)\s*(\d+)$`)
...
func IsDndDice(msg *gotgbot.Message) bool {
	if msg.Chat.Type != "private" && !GetGroupInfo(msg.Chat.Id).AutoCalculate {
		return false
	}
...
func DndHelp(bot *gotgbot.Bot, ctx *ext.Context) (err error) {
	helpText := `<b>骰子功能</b>
<b>用法：</b>
1. [骰子数量]d[骰子面数] [+/-调整值]
2. [骰子数量]b[骰子面数] [+/-调整值]
