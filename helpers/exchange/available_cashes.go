...
var availStr = `CNY,AED,AFN,ALL,AMD,ANG,AOA,ARS,AUD,AWG,AZN,BAM,BBD,BDT,BGN,BHD,BIF,
BMD,BND,BOB,BRL,BSD,BTN,BWP,BYN,BZD,CAD,CDF,CHF,CLP,COP,CRC,CUP,CVE,
...
var availableCashes = func() map[string]struct{} {
	avail := map[string]struct{}{}
	availStr = strings.ReplaceAll(availStr, "\n", "")
	for _, s := range strings.Split(availStr, ",") {
		avail[s] = struct{}{}
	}
	return avail
}()

func IsAvailableCash(cash string) bool {
	_, ok := availableCashes[cash]
	return ok
}
